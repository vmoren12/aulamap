...
(async () => {
  ...
  try {
    ...
    const beforeDelete = await page.evaluate(() => JSON.stringify(getData()));
    await page.evaluate(() => {
      getData().desks.slice(0, 2).forEach(desk => tableSelection.add(desk.id));
      refreshTableSelection();
    });
    await page.click('#deleteSelectedBtn');
    assert.equal(await page.evaluate(() => getData().desks.length), 18);
    assert.equal(await page.evaluate(() => getData().students.length), 12);
    await page.evaluate(() => undo());
    assert.equal(await page.evaluate(() => JSON.stringify(getData())), beforeDelete);
    await page.evaluate(() => {
      getData().desks.forEach(desk => tableSelection.add(desk.id));
      refreshTableSelection();
      el('canvasArea').focus();
    });
    ...
    const formed = await page.evaluate(() => ({ desks: getTeamLayout().desks, groups: getTeams().groups, assignments: getTeamLayout().assignments }));
    assert.equal(formed.desks.length, 12);
    assert.deepEqual(await page.evaluate(() => getData().assignments), beforeMode.assignments);
    ...
    await page.evaluate(() => {
      clearTableSelection();
      getTeamLayout().desks.slice(0, 2).forEach(desk => tableSelection.add(desk.id));
      refreshTableSelection();
    });
    await page.click('#deleteSelectedBtn');
    assert.equal(await page.evaluate(() => getTeamLayout().desks.length), 10);
    assert.deepEqual(await page.evaluate(() => getTeams().groups), formed.groups);
    assert.deepEqual(await page.evaluate(() => getData().desks), beforeMode.desks);
    await page.evaluate(() => undo());
    await page.evaluate(() => clearTableSelection());
    const studentToMove = await page.evaluate(() => getTeamLayout().assignments[desksForTeam(0)[0].id]);
    ...
    const selected = await page.evaluate(() => selectedTeamStudents());
    ...
    await page.mouse.up();
    assert.equal(await page.evaluate(ids => ids.every(id => getTeams().groups[1].includes(id)), selected), true);
    assert.equal(await page.evaluate(() => undoStack.length), undoCount + 1);
    assert.deepEqual(await page.evaluate(() => getData().desks), beforeMode.desks);
    await page.evaluate(() => undo());
    assert.deepEqual(await page.evaluate(() => getTeams().groups), formed.groups);
    await page.evaluate(() => {
      desksForTeam(0).slice(0, 2).forEach(desk => tableSelection.add(desk.id));
      refreshTableSelection();
    });
    await page.focus('#moveSelectedTeam');
    await page.keyboard.press('Delete');
    assert.equal(await page.evaluate(() => getTeamLayout().desks.length), 12);
    await page.select('#moveSelectedTeam', '2');
    assert.equal(await page.evaluate(() => getTeams().groups[2].length), 6);
    await page.evaluate(() => undo());
    ...
    assert.deepEqual(await page.evaluate(() => getTeams().groups), formed.groups);
    const firstLayout = await page.evaluate(() => getTeamLayout());
    await page.evaluate(() => {
      getTeamLayout().desks[0].x += 70;
      appendSavedTeam('Segona materia');
    });
    const secondLayout = await page.evaluate(() => getTeamLayout());
    await page.evaluate(() => loadSavedTeam(0));
    assert.deepEqual(await page.evaluate(() => getTeamLayout()), firstLayout);
    await page.evaluate(() => {
      const teams = getTeams();
      teams.saved.push({ name: 'Legacy', groups: teams.groups.map(group => [...group]), teamNames: {} });
      loadSavedTeam(teams.saved.length - 1);
    });
    assert.equal(await page.evaluate(() => teamsHaveUnsavedChanges()), false);
    await page.evaluate(() => { getTeamLayout().desks[0].x += 10; });
    assert.equal(await page.evaluate(() => teamsHaveUnsavedChanges()), true);
    await page.evaluate(() => { getTeamLayout().desks[0].x -= 10; loadSavedTeam(0); });
    await page.evaluate(() => loadSavedTeam(1));
    assert.deepEqual(await page.evaluate(() => getTeamLayout()), secondLayout);
    await page.evaluate(() => loadSavedTeam(0));
    await page.evaluate(() => {
